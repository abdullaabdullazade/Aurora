...
**A Flutter music player with a soft-dark glass design, backed by a Python resolver server.**

Real YouTube search & streaming · synced lyrics · offline downloads · on-device library · playlists
...
| Home | Now Playing | Synced lyrics | Shareable lyric card |
|---|---|---|---|
...
All frames: [`docs/media/`](docs/media)

---

## Why a server

Client-side scraping gets rate-limited and `403`'d per device IP, and breaks every time
YouTube changes. `yt-dlp` is the most robust extractor available, so Aurora runs it
server-side: one stable IP, a shared cache, and a Range-seekable audio proxy. The app never
talks to `googlevideo` directly.
...
- **Live autocomplete** from YouTube's own suggestion endpoint, plus a persisted **search history**.
- Home dashboard: parallax `SliverAppBar` header, carousels for Trending, Top Charts,
  Recently played and Quick downloads.
- Every section has three states at the same height — shimmer skeleton, empty card, error card
  with a **Retry pill** that refetches only that carousel. Nothing jumps when a future resolves.

### Playback
- Audio streams through the proxy as `audio/mp4` with HTTP Range, so seeking is instant.
- Remote YouTube and local device files run through the same `just_audio` engine.
...
- 5-band equalizer, playback speed, output picker, right-edge volume drag.

### Now Playing
- Full-screen layout: blurred artwork behind a colour veil derived from the cover.
- **Two-role dynamic colour** — a vivid *accent* for marks and a deep same-hue *backdrop*
  for the wash, each held to its WCAG ratio. See [Colour](#colour).
- Custom **waveform seeker** with an elastic bump and a haptic tick per bar.
- Particle pulse around the album art that breathes faster while playing.
- **Synced lyrics** (lrclib) that open at and follow the current line; tap a line to seek.
...
### Offline
- Connectivity is watched live. Offline, non-downloaded items fade to 40 % and stop responding,
  and a breathing "Offline Sanctuary" pill appears under the header.
...
| `Tone.onColor` | black or white, whichever scores higher | legible glyph on any accent |

Glass surfaces put a dark scrim under their white sheen — `BackdropFilter` preserves the colour
behind it, so a white-only fill turns into a bright smear over album art.
...
Type is Plus Jakarta Sans on a tight scale (`AppType`); spacing and radii come off a 4-pt scale
(`Sp`, `Radii`); routes fade through with a slight upward slide.

---

## Architecture

Clean Architecture with Riverpod. Presentation depends only on domain abstractions, entities are
immutable, and no business logic lives in a view.
...
**Stack** — `flutter_riverpod` · `just_audio` · `audio_service` · `palette_generator` · `hive` ·
...
- True crossfade (two overlapping players) — today's is a fade-out into a fade-in, because
  `just_audio_background` accepts only one player instance.
